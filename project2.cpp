...
const char *WINDOWTITLE = { "Project 2: Animate a Helicopter! -- Chelsea Li" };
...
int		WhichView;				// outside or inside view
int		Xmouse, Ymouse;			// mouse values
float	Xrot, Yrot;				// rotation angles in degrees

#include "heli.550.h"

#define BLADE_RADIUS		 (float)1.0
#define BLADE_WIDTH		 (float)0.4
...
Animate( )
{
	...
	Time = ((float)ms  /  (float)MS_IN_THE_ANIMATION_CYCLE) * 5000;        // [ 0., 1. )
...
Display( )
{
	...
	// set the viewing volume: to perspective
	...
	{
		gluLookAt( 15., (float)1.8, -10.,     -30., 0., 0.,     0., 1., 0. );
	...
	} else if ( WhichView == INSIDE) {
		gluLookAt( (float)-0.4, (float)1.8, (float)-4.9,     0., 1., -12.,     0., 1., 0. );
	}
	...
	// top helicopter blade

	glPushMatrix( );
	glTranslatef( 0., (float)2.75, -2. );
	glScalef( 5., 5., 5. );
	glRotatef( 90., 1., 0., 0 );
	...
	glPushMatrix( );
	glTranslatef( (float)0.5, (float)2.5, 9. );
	glScalef( 3., 3., 3. );
	glRotatef( 270., 0., 1., 0. );
...
InitLists( )
{
	...
	glNewList( BoxList, GL_COMPILE );
		...
		glBegin( GL_QUADS );
			glVertex3f( -2., (float)-0.2, -14. );
			glVertex3f( 2., (float)-0.2, -14. );
			glVertex3f( 2., (float)3.8, -14. );
			glVertex3f( -2., (float)3.8, -14. );

			glVertex3f( -2., (float)-0.2, -18. );
			glVertex3f( -2., (float)3.8, -18. );
			glVertex3f( -2., (float)3.8, -18. );
			glVertex3f( -2., (float)-0.2, -18. );
