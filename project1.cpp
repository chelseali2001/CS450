...
const char *WINDOWTITLE = { "Project 1 -- Chelsea Li" };
...
GLuint	UmbrellaList;				// object display list
...
InitLists( )
{
	// coordinates for the vertices on the umbrella

	float dang = (float)(2. * 3.14 / (NUMSEGS - 1));
	float ang = 0.;
	float height = 1.;
	...
	UmbrellaList = glGenLists( 1 );
	glNewList( UmbrellaList, GL_COMPILE );

		// drawing the outline of the umbrella
		...
		for (double i = 0; i < 1; i += 0.01) {
			glBegin( GL_LINE_LOOP );
				...
				for (int x = 0; x < NUMSEGS; x++) {
					glVertex3f((float)(i*cos(ang)), (float)(height - i), (float)(i*sin(ang)));
					ang += dang;
				}
			glEnd( );
		}

		// coloring the umbrella

		for (double i = 0; i < 0.99; i += 0.01) {
			float prevTopX = (float)(0*cos(ang));
			float prevTopY = (float)height;
			float prevTopZ = (float)(0*sin(ang));
			float prevBottomX = (float)(0.05*cos(ang));
			float prevBottomY = (float)(height - 0.05);
			float prevBottomZ = (float)(0.05*sin(ang));
			int col = 0;
			ang = 0.;
			...
			glBegin( GL_QUADS );
				for (int x = 0; x < NUMSEGS; x++) {
					...
					// different sections have different colors
					...
					glVertex3f(prevTopX, prevTopY, prevTopZ);
					glVertex3f(prevBottomX, prevBottomY, prevBottomZ);
					glVertex3f((float)(i*cos(ang)), (float)(height - i), (float)(i*sin(ang)));
					glVertex3f((float)((i + 0.05)*cos(ang)), (float)(height - i - 0.05), (float)((i + 0.05)*sin(ang)));

					prevTopX = (float)(i*cos(ang));
					prevTopY = (float)(height - i);
					prevTopZ = (float)(i*sin(ang));
					prevBottomX = (float)((i + 0.05)*cos(ang));
					prevBottomY = (float)(height - i - 0.05);
					prevBottomZ = (float)((i + 0.05)*sin(ang));
					ang += dang;
				}
			glEnd( );
		}
