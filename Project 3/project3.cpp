...
const char *WINDOWTITLE = { "Project 3: Texture Mapping -- Chelsea Li" };
...
int		WhichTexture;			// no texture, use texture image, or distort texture image
...
Display( )
{
	...
	if ( WhichTexture == DISTORTION ) { // distortion option animates the distortion (picture should stretch in and out)
		...
		OsuSphere( 1., 64, 64 );
		glPopMatrix( );
	} else if ( WhichTexture == TEXTURE_IMAGE ) { // normal texture image option
		glEnable( GL_TEXTURE_2D );
		glPushMatrix( );
		OsuSphere( 1., 64, 64 );
		glPopMatrix( );
	} else { // no texture (just makes a white ball)
...
InitMenus( )
{
	...
	glutAddMenuEntry( "Texture image",   TEXTURE_IMAGE );
	glutAddMenuEntry( "Distort texture image",	DISTORTION );
...
{	
	...
	unsigned char *t32 = BmpToTexture( "worldtex.bmp", &width, &height );
...
OsuSphere( float radius, int slices, int stacks )
{
	...
	for( int ilat = 0; ilat < NumLats; ilat++ )
	{
											...
											// ilng=NumLngs-1, lng=+M_PI are the same meridian
		{
			...
			p->ny = y;
			p->nz = z;
			if ( WhichTexture == DISTORTION ) { // "messed up" coordinates for distortion option
				p->s = ( lng + 1    ) / ( 2.*2 );
				p->t = ( lat + 3/2. ) / 4;
			} else {
				p->s = ( lng + M_PI    ) / ( 2.*M_PI );
